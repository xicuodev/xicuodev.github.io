...
变量的类型在编译时（或声明时）确定，需要显式声明。例如 C/C++ 是静态类型语言的典型代表，其他的静态类型语言还有 C#、Java 等。
...
变量是类型安全的，不允许任何隐式不安全转换。例如 C/C++、Java、C#。

定义一个整型变量 a，若不显示转换，不能将 a 当作字符串类型处理。

## 弱类型定义语言 Implicit type conversion

弱变量是类型不安全的，允许大量的隐式不安全转换。例如 PHP、ASP、Ruby、Python、Perl、ABAP、SQL、JavaScript、Unix Shell 等。

在 VBScript 中，可以将字符串 '12' 和整数 3 进行连接得到字符串 '123'，  然后可以把它看成整数 123，而不需要显示转换。
